...
void NoximRouterPlus::rxProcess(){
	int i,j,k;
...
void NoximRouterPlus::txProcess(){
	if (NoximGlobalParams::verbose_mode > VERBOSE_LOW ) 
...
int NoximRouterPlus::Detour(const NoximRouteData & route_data, int input, int waiting)
{//	assert(false);
...
NoximNoP_data NoximRouterPlus::getCurrentNoPData() const
{
...
void NoximRouterPlus::bufferMonitor()
{	
	int i;
    if (reset.read()) {
...
vector < int >NoximRouterPlus::routingFunction(const NoximRouteData & route_data)
{
    NoximCoord position  = id2Coord(route_data.current_id);
    NoximCoord src_coord = id2Coord(route_data.src_id    );
    NoximCoord dst_coord = id2Coord(route_data.dst_id    );
...
int NoximRouterPlus::route(const NoximRouteData & route_data)
{
    if (route_data.dst_id == local_id && route_data.arr_mid)
		return DIRECTION_LOCAL;
...
void NoximRouterPlus::NoP_report() const
{
...
void NoximRouterPlus::RCA_Aggregation()
{
	int i,j;
	if(reset.read()){
		for(i=0; i < DIRECTIONS; i++)
...
int NoximRouterPlus::NoPScore(const NoximNoP_data & nop_data,
...
int NoximRouterPlus::selectionNoP(const vector < int >&directions,
...
int NoximRouterPlus::selectionBufferLevel(const vector < int >&directions)
{
...
int NoximRouterPlus::selectionRCA2D(const vector<int>& directions, const NoximRouteData& route_data){
	  vector<int>  best_dirs;
...
int NoximRouterPlus::selectionProposed(const vector <int> & directions, const NoximRouteData & route_data)
{
	vector < int >neighbors_on_path;
    vector < int >score;
    int direction_selected = NOT_VALID;
...
int NoximRouterPlus::selectionThermal(const vector<int>& directions, const NoximRouteData& route_data){ //revised by 20130803  //Thermal delay based
...
int NoximRouterPlus::selectionRandom(const vector < int >&directions)
{
    return directions[rand() % directions.size()];
}
vector<int> NoximRouterPlus::DW_layerSelFunction    (const int select_routing, const NoximCoord& current, const NoximCoord& destination, const NoximCoord& source, int dw_layer){
...
int NoximRouterPlus::selectionFunction(const vector < int >&directions,
				   const NoximRouteData & route_data)
{
...
vector < int >NoximRouterPlus::routingXYZ(const NoximCoord & current,const NoximCoord & destination){
...
vector<int> NoximRouterPlus::routingZXY(const NoximCoord& current, const NoximCoord& destination){
  vector<int> directions;
...
vector<int> NoximRouterPlus::routingDownward(const NoximCoord& current, const NoximCoord& source, const NoximCoord& destination){
...
vector<int> NoximRouterPlus::routingWF_Downward(const NoximCoord& current, const NoximCoord& source, const NoximCoord& destination){
...
vector < int >NoximRouterPlus::routingWestFirst(const NoximCoord & current,
...
vector < int >NoximRouterPlus::routingNorthLast(const NoximCoord & current,
					    const NoximCoord & destination)
{
    vector < int >directions;
...
vector < int >NoximRouterPlus::routingNegativeFirst(const NoximCoord & current,
						const NoximCoord &
						destination)
{
    vector < int >directions;
...
vector < int >NoximRouterPlus::routingOddEven(const NoximCoord & current,
...
/*=============================Odd Even-3D==========================*/
vector<int> NoximRouterPlus::routingOddEven_for_3D(const NoximCoord& current, 
...
vector<int> NoximRouterPlus::routingOddEven_3D (const NoximCoord& current, 
				    const NoximCoord& source, const NoximCoord& destination)
{
  vector<int> directions;
...
vector<int> NoximRouterPlus::routingOddEven_Downward (const NoximCoord& current, 
...
vector<int> NoximRouterPlus::routingOddEven_Z (const NoximCoord& current, 
				    const NoximCoord& source, const NoximCoord& destination)
{
	vector<int> directions;
...
vector < int >NoximRouterPlus::routingDyAD(const NoximCoord & current,
				       const NoximCoord & source,
				       const NoximCoord & destination)
{
    vector < int >directions;
...
vector<int> NoximRouterPlus::routingFullyAdaptive   (const NoximCoord& current                          , const NoximCoord& destination){
    vector < int >directions;
/*
    if        (destination.x == current.x || destination.y == current.y)
		return routingXYZ(current, destination);
...
vector<int> NoximRouterPlus::routingTableBased      (const NoximCoord& current, const int dir_in        , const NoximCoord& destination){
...
vector<int> NoximRouterPlus::routingDLADR           (const NoximCoord& current, const NoximCoord& source, const NoximCoord& destination,const int select_routing, int dw_layer){
...
vector<int> NoximRouterPlus::routingDLAR            (const NoximCoord& current, const NoximCoord& source, const NoximCoord& destination,const int select_routing, int dw_layer){
	switch ( select_routing ){
...
vector<int> NoximRouterPlus::routingDLDR            (const NoximCoord& current, const NoximCoord& source, const NoximCoord& destination,const int select_routing, int dw_layer){
	switch ( select_routing ){
	case ROUTING_XYZ                 :return routingXYZ         (                 current, destination                   );break;
...
vector<int> NoximRouterPlus::routingTLAR_DW         (const NoximCoord& current, const NoximCoord& source, const NoximCoord& destination){
	vector<int> directions;
...
vector<int> NoximRouterPlus::routingTLAR_DW_VBDR    (const NoximCoord& current, const NoximCoord& source, const NoximCoord& destination){
	vector<int> directions;
	int max = 0;
	int layer = 3;
	int free_slot[4];
...
vector<int> NoximRouterPlus::routingTLAR_DW_ODWL_IPD(const NoximCoord& current, const NoximCoord& source, const NoximCoord& destination, int dw_layer){
	vector<int> directions;
	int layer = dw_layer;
	if(current.z < layer && (current.x != destination.x || current.y != destination.y) ){
...
vector<int> NoximRouterPlus::routingTLAR_DW_ODWL    (const NoximCoord& current, const NoximCoord& source, const NoximCoord& destination, int dw_layer){
	vector<int> directions;
	int layer = dw_layer;
...
vector<int> NoximRouterPlus::routingTLAR_DW_IPD     (const NoximCoord& current, const NoximCoord& source, const NoximCoord& destination){
	vector<int> directions;
...
void NoximRouterPlus::configure(const int _id,
...
void NoximRouterPlus::TBDB(float consumption_rate)
...
unsigned long NoximRouterPlus::getRoutedFlits()
{
...
unsigned long NoximRouterPlus::getRoutedDWFlits()
{
	return routed_DWflits;
}

unsigned long NoximRouterPlus::getRoutedFlits(int i)
{
    return routed_flits[i];
}

unsigned long NoximRouterPlus::getWaitingTime(int i)
{
	return waiting[i];
}
unsigned long NoximRouterPlus::getTotalWaitingTime()
{
	return _total_waiting;
} 
unsigned long NoximRouterPlus::getRoutedPackets()
{
	return routed_packets;
}
void NoximRouterPlus::CalcDelay(){
	_buffer_pkt_count = 0;
...
int NoximRouterPlus::getFlitRoute(int i){
	NoximFlit flit = buffer[i].Front();
	NoximRouteData route_data;
	route_data.current_id = local_id;
	route_data.src_id     = (flit.arr_mid)?flit.mid_id:flit.src_id;
...
int NoximRouterPlus::getDirAvailable(int i){
	
	return reservation_table.isAvailable(i);
	
}

double NoximRouterPlus::getPower()
{
...
int NoximRouterPlus::reflexDirection(int direction) const
{
    switch( direction ){
...
int NoximRouterPlus::getNeighborId(int _id, int direction) const
{
...
void NoximRouterPlus::TraffThrottlingProcess()	//若在emergency mode, 且traffic超過traffic quota, 則throttle
{
...
void NoximRouterPlus::IntoEmergency(){
	_emergency  = true;
	if(_emergency_level < NoximGlobalParams::buffer_depth-1)
		_emergency_level++;
}

void NoximRouterPlus::OutOfEmergency(){
	_emergency       = false;
	if(_emergency_level > 0)
		_emergency_level--;
}

bool NoximRouterPlus::inCongestion()
{
    for (int i = 0; i < DIRECTIONS; i++) {
...
bool NoximRouterPlus::Adaptive_ok( NoximCoord & sour, NoximCoord & dest)
{
...
void  NoximRouterPlus::DBA( int outgoing,NoximFlit* head){
	NoximCoord local = id2Coord(local_id);
